...
\end{align*}
where $f(z) : \mathcal{X}^s \to H^{s - 1}(M)$ is Fredhom and compact for any $z \in \C$. Consequently, by the analytic Fredholm theorem \ref{} \todo{mention in functional analysis section} we have that either $1 - f(z)$ is not invertible for any $z \in \C$ or $(1 - f(z))^{-1}$ is a meromorphic function whose poles give rise to non-trivial solution 
\begin{align*}
f(z) u = (\Box - iQ + zA) u = u. 
\end{align*}
