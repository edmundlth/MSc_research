...
\todo{Historical note about H\"ormander symbol class. Vasy's definition. Melrose generalisation etc... }
...
\begin{rem} \hfill 
    \begin{itemize}
        ...
        a \sim \sum_{j} a_{m - j}
        \end{align*} 
        for some $a_{m -j} \in S^{m}_\infty(\R^{p}; \R^n)$ that are homogeneous of order $m - j$  in $\xi$, i.e. $a_{m, j}(x, t \xi) = t^{m - j}a_{m - j}(x, \xi)$ whenever $t \in \R_{> 0}$, $\abs{\xi} \geq 1$. The sum above are taken in the sense of (\ref{def : asymptotic sumation}). We note that the characteristic polynomial of any differential operator is a polyhomogeneous symbol. We will denote the pseudodifferential operators that arrises from polyhomogeneous symbols as classical pseudodifferential operators, $\Psi^{m}_{cl}(\R^n)$. 
        
         \todo{polyhomogeneous subspace}
    \end{itemize}
\end{rem}
...
\begin{flemma}\label{lemma: density of residual symbols}
...
%Explicitly, we want to show that for each $a \in S^{m}_\infty(\R^{2n}; \R^n)$
%\begin{align*}
...
\begin{fdefinition}[Asymptotic summation] \label{def : asymptotic sumation}
